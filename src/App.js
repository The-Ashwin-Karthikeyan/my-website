...
function App() {
  ...
  // For detecting long press on mobile
  const touchStartRef = useRef(null);

  useEffect(() => {
    const handleKeyDown = (e) => {
      // Check if CTRL is pressed and the key is backtick (`), which is the unshifted ~ key
      if (e.ctrlKey && e.key === '`') {
  ...
  const handleTouchEnd = () => {
    ...
    // If touch was 2 seconds (2000 ms) or more, toggle the terminal
    if (holdTime >= 4000) {
      setShowTerminal((prev) => !prev);
    }
  };
